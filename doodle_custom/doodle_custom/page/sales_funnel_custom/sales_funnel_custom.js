...
erpnext.SalesFunnel = class SalesFunnel {
	...
	setup(wrapper) {
		...
		//Divison Filter Adding 
		this.division_field = wrapper.page.add_field({"fieldtype": "Link", "fieldname": "division", "options": "Cost Center",
			"label": __("Division"),"reqd": 1,
			change: function() {
				me.division = this.value;
				me.get_data();
			}
		}),
	...
	get_data(btn) {
		...
		if (!this.division) {
			frappe.throw(__("Please Select a Division."));
		}

		const method_map = {
			"sales_funnel": "doodle_custom.doodle_custom.page.sales_funnel_custom.sales_funnel_custom.get_funnel_data",
