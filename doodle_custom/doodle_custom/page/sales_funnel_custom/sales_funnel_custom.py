...
def validate_filters(from_date, to_date, company, division):
	...
	if not division:
		frappe.throw(_("Please Select a Divison"))


@frappe.whitelist()
def get_funnel_data(from_date, to_date, company, division):
	validate_filters(from_date, to_date, company, division)

	active_leads = frappe.db.sql(
		"""select count(*) from `tabLead`
		where (date(`creation`) between %s and %s)
		and company=%s and division=%s""",
		(from_date, to_date, company, division),
	)[0][0]

	opportunities = frappe.db.sql(
		"""select count(*) from `tabOpportunity`
		where (date(`creation`) between %s and %s)
		and opportunity_from='Lead' and company=%s and division=%s""",
		(from_date, to_date, company, division),
	)[0][0]
	...
	quotations = frappe.db.sql(
		...
		and (opportunity!="" or quotation_to="Lead") and company=%s and division=%s""",
		(from_date, to_date, company, division),
	)[0][0]
	...
	converted = frappe.db.sql(
		...
		and `tabLead`.company=%s and `tabLead`.division=%s""",
		(from_date, to_date, company, division),
	)[0][0]
