...
class CustomSubscription(Subscription):
	...
	def is_past_grace_period(self):
		...
		if self.current_invoice_is_past_due(current_invoice):
			...
			return getdate() > add_days(self.start_date, grace_period)
	...
	def current_invoice_is_past_due(self, current_invoice=None):
		...
		if not current_invoice or self.is_paid(current_invoice):
			...
			# return getdate() > getdate(current_invoice.delivery_date)
			pass
	...
	def get_current_invoice(self):
		...
		"""
		doctype = "Sales Order" if self.party_type == "Customer" else "Purchase Invoice"
	...
	def create_invoice(self, prorate):
		...
		if doctype == "Sales Order":
			# invoice.naming_series = 'INV-D-'
			if invoice.company == "Doodle Technologies LLC Dubai":
				invoice.delivery_date = frappe.utils.add_days(self.current_invoice_start, 0)
		...
		if doctype == "Sales Order" and self.sales_tax_template:
		...
		if self.days_until_due:
			invoice.append(
				"payment_schedule",
				{
					"due_date": add_days(invoice.transaction_date, cint(self.days_until_due)),
					"invoice_portion": 100,
				},
			)
	...
	def is_current_invoice_generated(self, _current_start_date=None, _current_end_date=None):
		...
		if invoice and getdate(_current_start_date) <= getdate(invoice.transaction_date) <= getdate(
			_current_end_date
		):
			return True
	...
	def has_outstanding_invoice(self):
		...
		outstanding_invoices = frappe.get_all(
			doctype, fields=["name"], filters={"status": ("!=", "Closed"), "name": ("in", invoice_list)}
		)
