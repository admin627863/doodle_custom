...
def scrap_asset(asset_name):
	...
	frappe.msgprint(_("Custom Asset scrapped via Journal Entry {0}").format(je.name))
...
def restore_asset(asset_name):
    ...
    # Reverse depreciation and reset schedule
    ...
    # Reset disposal details
    asset.db_set("disposal_date", None)
    asset.db_set("journal_entry_for_scrap", None)

    # Handle Journal Entry: Cancel if submitted, delete if draft
    if je and frappe.db.exists("Journal Entry", je):
        je_doc = frappe.get_doc("Journal Entry", je)
        
        if je_doc.docstatus == 1:
            # If submitted, cancel the Journal Entry
            je_doc.cancel()
            frappe.msgprint(f"⚠ Journal Entry {je} has been cancelled.", alert=True)
        else:
            # If draft, delete the Journal Entry
            frappe.delete_doc("Journal Entry", je, force=True)
            frappe.msgprint(f"✅ Journal Entry {je} has been permanently deleted.", alert=True)
